...
open class Floaty: UIView {
    ...
    @objc open var paddingY: CGFloat = UIScreen.main.bounds.size.height - 70 {
        didSet {
            self.setNeedsDisplay()
        }
    }
     ...
     */
    @IBInspectable
    @objc open var buttonColor: UIColor = UIColor(red: 68/255.0, green: 72/255.0, blue: 112/255.0, alpha: 1)
    
    /**
     Button image.
     */
    @IBInspectable
    @objc open var buttonImage: UIImage? = UIImage(named: "menuIcon") {
        didSet {
            self.setNeedsDisplay()
        }
    }
        ...
        if(items.count > 0){
            ...
            UIView.animate(withDuration: 0.3, delay: 0,
                           ...
                           options: [], animations: { () -> Void in
                            ...
                            self.buttonImage = UIImage(named: "menuIcon")
                            self.buttonColor = UIColor(red: 68/255.0, green: 72/255.0, blue: 112/255.0, alpha: 1)
                            self.overlayView.alpha = 0
            }, completion: {(f) -> Void in
